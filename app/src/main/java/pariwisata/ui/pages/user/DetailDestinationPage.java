...
public class DetailDestinationPage {
    ...
    public Parent getView(Stage stage, Destination destination) {
        ...
        // Bind image width dynamically to the content width minus padding to prevent horizontal scroll
        ...
        // Harga
        HBox priceBox = buildInfoChip("ic-price", "Harga : " + destination.getPrice(), "#0D6EFD", "#EAF4FF");
    ...
    private String getPlatformIcon(String platform) {
        if (platform == null) return "ic-wisata";
        String p = platform.toLowerCase();
        if (p.contains("instagram")) return "ic-instagram";
        if (p.contains("facebook"))  return "ic-facebook";
        if (p.contains("youtube"))   return "ic-youtube";
        if (p.contains("tiktok"))    return "ic-tiktok";
        return "ic-wisata";
    }
    ...
    private VBox buildReviewCard(Review review) {
        ...
        for (int i = 0; i < 5; i++) stars.append(i < review.getRating() ? "★" : "☆");
    ...
    private void loadDetailImage(ImageView imageView, String photoUrl) {
        ...
        Thread thread = new Thread(() -> {
            try {
                // Load preserving ratio to avoid scaling issues and let the fitWidth bind do the work
